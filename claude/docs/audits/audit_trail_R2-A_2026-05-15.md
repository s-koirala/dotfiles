...
| P2A-1-2 | minor | `cwd_is_publishing` used substring match for `project-skie` token, which would false-positive on `my-project-skie/`, `old-project-skie-archive/`, etc. Rule glob is `**/project-skie/**` (exact segment, no wildcards). | **fixed**: split into `_PUBLISHING_EXACT = ('project-skie',)` (exact segment) and `_PUBLISHING_SUBSTRING = ('publication','manuscript')` (rule uses `*foo*`); separate checks for each. Verified on 6 path test cases. |
...
- Publishing-cwd detection: PASS on 6/6 edge cases including `my-project-skie/` correctly NOT matching
