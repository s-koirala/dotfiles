...
| Python hooks (pre/post tool use, session start/end, pre-commit) | 7 | `hooks/` |
| Path-scoped rule files | 2 | `rules/` |
| Top-level Python scripts | 4 | `scripts/` |
| Procedural skills | 14 | `skills/` |
| Reusable templates (ADR, DAG, DUA, schemas) | 5 | `templates/` |
| Bootstrap-project sub-templates (CLAUDE.md.tmpl, README.md.tmpl, pyproject.toml.tmpl, kind-specific, etc.) | 10 | `scripts/bootstrap_templates/` |
| Audit trails (design + implementation) | 16+ | `docs/audits/` |
...
| `emit-repro-log` | 13-field ReproLog JSON record (reproducibility envelope) |
...
| `pre-register-hypothesis` | Freeze design.md with SHA-256 anchor; 11-section pre-registration template |
...
`session_start_provenance`, `session_end_audit_log`, `pre_write_seed_guard`, `precommit_seed_guard`, `pre_bash_safety`, `post_write_notebook_clean`, `pre_write_phi_guard`.
