...
The leakage taxonomy is grounded in López de Prado 2018 *Advances in Financial Machine Learning* §7 ("Cross-Validation in Finance"), which introduces purge + embargo as the structural defense against horizon-overlap leakage but does NOT itself prescribe the inject-the-future-feature canary technique. The three concrete canaries below operationalize the AFML §7 framework as an inject-the-future-feature diagnostic:
...
- p-value threshold: **p < 0.01**  # justify: conservative because false negative = silent leak in production (high cost) and false positive = re-investigate (low cost). For a project-specific tighter or looser threshold, override in the project's `config/pit_canary.yaml`.
...
1. **Locate the project's backtest config and feature set** (e.g. `config/hypotheses/<HID>.yaml` or the project's hypothesis-config convention).
